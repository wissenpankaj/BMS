...
(
    replenishment_id bigint NOT NULL GENERATED BY DEFAULT AS IDENTITY (INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9223372036854775807 CACHE 1),  -- Identity column for replenishment_id
    created_at timestamp(6) without time zone DEFAULT CURRENT_TIMESTAMP,  -- Default current timestamp for created_at
    station_id bigint NOT NULL,  -- Foreign key for station_id
    requested_stock integer NOT NULL,  -- Requested stock quantity
    CONSTRAINT replenishment_pkey PRIMARY KEY (replenishment_id),  -- Primary key constraint
    CONSTRAINT fk_station FOREIGN KEY (station_id)  -- Foreign key constraint referencing station table
...
(
    ...
    battery_id bigint,
    CONSTRAINT batteries_stock_pkey PRIMARY KEY (id)
);
